...
返値（通常）:      {count: N, ids: [...], server_time}
返値（include_read=true）: {count: N, unread_count: N2, ids: [...],
                             messages: [{id, read, persistent, title, from, to}, ...], server_time}
...
引数: backend（省略可 — "lmstudio" or "anthropic"。省略時は ANTHROPIC_API_KEY があれば anthropic、なければ lmstudio）
       ...
       status_only（省略可, bool — 起動せず進捗と未処理raw件数だけ返す）
返値（起動時）: {started: true, backend, force, raw_pending, server_time}
返値（status_only）: {running, total, processed, errors, skipped, raw_pending, ..., server_time}
...
| [docs/data_structure.md](docs/data_structure.md) | Claude エクスポート ZIP のデータ構造仕様 |
