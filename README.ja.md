...
**設計フェーズ**
- OpenWebUI 会話ログ同期 — ローカル LLM (LMStudio + OpenWebUI) のチャット履歴を mio-memory に取り込み、Claude.ai ログと統合検索（[設計書](docs/openwebui-sync.ja.md)）
- SysMemory ダンプの世代管理
...
- MCPリクエストログ＋instructions強化（v3.58）— MCPエンドポイント（`/mcp`）に構造化アクセスログ追加：User-Agent / ソースIP / MCPメソッド名 / クライアント種別推定（desktop-app / browser / ipad / mobile / claude-code / anthropic-cloud / script）を `MCP-ACCESS:` プレフィックスで出力。PC環境のMCPコネクタ不通問題（M-PC1）の切り分け支援用。MCP initialize の `instructions` を具体的な用途記述に拡充し、Claude.ai の tool_search に拾われやすくした
