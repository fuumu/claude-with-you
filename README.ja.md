...
返値: {sublimated, rating, rating_reason, chunks, attempts, needs_human, model, uuid?}
※ 昇華 = 体験の温度・感情・意味を保ったまま、性的・身体的な行為の明示的描写を抽象化・詩化する変換
  （rating_policy.md 準拠・mature 以下に収める）。バカンス生日記の代理昇華・生ログからの日記起こし用
※ セルフチェックループ: 昇華出力をレーティング判定に通し、adult なら理由をフィードバックして
  再昇華（上限2回）。それでも adult なら needs_human=true で返す
