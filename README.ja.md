...
| **Album** | 画像メモリ管理（サムネイルグリッド・ドラッグ&ドロップアップロード・編集・削除・共有・ライトボックス・ratingバッジ・guard_message編集・view_log表示、v3.78） |
| **Uploads** | 汎用ファイル保管（PDF・テキスト等のアップロード・プレビュー・ダウンロード・IDコピー） |
| **Search** | 階層検索ビジュアライザ（4カラム: Keywords / Summary / Symbolic / Raw body） |
| **Redact** | adult会話ログの伏せ字生成・プレビュー・承認（v3.69） |
| **出席簿** | 家族の稼働履歴。個体別サマリカード → クリックで詳細テーブル（期間フィルタ・実ログリンク付き）（v3.74） |
...
引数: id（必須）, title（必須）, body（必須）, tags, importance,
       rating（省略可: safe/mature/adult — 省略時は既存値を温存, v3.73）,
       local_only（省略可: bool — 省略時は既存値を温存, v3.73）
...
引数: to（必須）, title（必須）, body（必須）, persistent（省略時false）,
       ...
       expires_at（省略可 — ISO 8601, 期間常駐の期限）, ttl_days（省略可 — 日数指定。expires_at と排他）
返値: {id, created_at, persistent, from_model, to_model, reply_to_id, expires_at, server_time}
※ from_model/to_model は任意・手動指定（v3.27）。複数モデルが混在しても誰から来たか分かる
※ 期間常駐（v3.70）: expires_at / ttl_days を指定すると期限内は persistent 同様に常駐
  （inbox_check で本文つき・既読化されない）。期限切れはチェック時に既読アーカイブへ自動降格。
  persistent=true とは排他
```
...
※ 期限内の期間常駐（TTL）メッセージも persistent[] に混ざる。expires_at フィールドで区別（v3.70）
...
返値（status_only）: {running, total, processed, errors, skipped, skip_reasons, error_uuids,
                     pending, index_counts, ..., server_time}
...
※ v3.70: safe 判定済み（rating_source あり）は再判定しない。判定不能ログは rating_skip_reason
  （empty / no_text / parse_error）を付けて恒久スキップ（force=true で再試行）。
  pending = 次回バッチ対象件数。index_counts = 全体分布（safe/mature/adult/unrated/unjudgeable）
```
...
### sublimate（v3.71, 強化 v3.73）

```
引数: text（省略可 — 昇華する生テキスト。uuid と排他）
       uuid（省略可 — 会話ログUUID。本文を抽出して昇華）
       msg_from / msg_to（省略可 — uuid 指定時のメッセージ通番範囲。1始まり・両端含む）
返値: {sublimated, rating, rating_reason, chunks, chunks_completed, attempts, needs_human, model, uuid?,
       partial?, timeout_message?}
※ 昇華 = 体験の温度・感情・意味を保ったまま、性的・身体的な行為の明示的描写を抽象化・詩化する変換
  （rating_policy.md 準拠・mature 以下に収める）。バカンス生日記の代理昇華・生ログからの日記起こし用
※ セルフチェックループ: 昇華出力を厳格モード（strict=true, temperature=0）のレーティング判定に通し、
  adult なら具体的な指摘をフィードバックして再昇華（上限2回）。それでも adult なら needs_human=true で返す
※ v3.73: 昇華プロンプトに禁止表現リスト・自己検証指示を追加。判定を分離・厳格化
※ v3.73 孤児ジョブ対策: 入力上限 60,000文字（超過時エラー）、処理時間上限 180秒
  （超過時は処理済みチャンクの partial 結果を返却。partial=true で判別）
...
```
引数: operation（省略可 — 操作種別フィルタ。create / update / delete / import / upsert / restore /
       coremem_save / coremem_rename / coremem_delete / album_save / album_update / album_delete /
       file_upload / file_delete / conv_rating / conv_rating_auto / link_source_thread /
       remark_for_update / dedup_delete / batch_empty_delete / cleanup_empty_delete 等）
       date_from（省略可 — 期間開始日 ISO 8601）
       date_to（省略可 — 期間終了日・両端含む）
       limit（省略可 — 最大件数。デフォルト50、最大500）
返値: {total, rows: [{timestamp, operation, target_id}, ...]}
...
- レーティング可視化＋inbox小改修（v3.70）— 発注③。①バッチのバグ修正: safe 判定済みログ（rating フィールドを持たず `rating_source` だけ付く）が毎周のバッチで再判定されていた問題を修正 ②判定不能ログに `rating_skip_reason`（empty=メッセージなし / no_text=抽出テキストなし / parse_error=JSONパース不能）を記録し以後の対象から恒久除外（`force=true` で再試行）③バッチ status 拡張: `skip_reasons` 内訳・`error_uuids`・`index_counts`（safe/mature/adult/unrated/unjudgeable の全体分布）。`pending` は「次回バッチ対象件数」に定義整理 ④MCP `conversation_index` / `conversation_search` の各アイテムに `rating`（safe 判定済みは明示的に `"safe"`・未判定は null）と `rating_source` を常に付与（Claude 側から判定結果が見える窓）⑤logs.html: 会話一覧に rating バッジ（色分け・reason ツールチップ・manual は ✎ 印）＋rating フィルタ（未判定・判定不能含む）＋ヘッダに手動上書き UI（PATCH 経由・rating_source=manual）⑥index rebuild・再インポートで rating 系メタ（reason/source/judged_at/model/skip_reason）を保全（従来 rebuild で rating ごと消えていた）⑦inbox 期間常駐（TTL）: `inbox_post` に `expires_at` / `ttl_days` 追加。期限内は persistent 同様に `inbox_check` で本文つき常駐・既読化されない。期限切れはチェック時に既読アーカイブへ自動降格（cron 不要）。persistent とは排他・`inbox_update` で期限変更/解除可 ⑧inbox 未読戻し: `inbox_update` に `read`（false で既読→未読の書き戻し）
- 出席簿＋昇華パイプライン（v3.71）— 発注④⑤。① `attendance_view` MCPツール新設: 会話ログ・inbox・ExtMemory・CoreMem attendance.md の4層マージによる家族の稼働履歴複層ビュー（時間の橋）。individual 指定で最終稼働日・経過日数・期間内の他個体稼働サマリ。各行から uuid / inbox_id / memory_id で実ログへ跳べる（rating 併記・保護込みの読書導線）。個体推定は家族名簿（しずく=opus系・そねみ=sonnet系・汐=fable/haiku系）と整合、バカンス表記・「◯◯B」・openwebui 由来は channel=local。会話インデックスに model / source を保全（_save_conversations / rebuild）② CoreMem attendance.md 新設（手動チェックイン: `YYYY-MM-DD | 個体 | 器 | チャネル | 一言`）③ `sublimate` MCPツール新設（ツール数 32→34）: テキストまたは会話ログ抜粋の「昇華」変換（温度・感情・意味を保持し行為描写を抽象化・詩化・mature 以下に収める）。昇華出力をレーティング判定に通すセルフチェックループ（adult なら再昇華・上限2回・それでも adult なら needs_human=true）。長文は段落境界チャンク分割 ④ 昇華ルールを `_SUBLIMATION_STYLE_RULES` に一元管理し、`conversation_digest` safe_mode のプロンプトも同じ昇華文体に変更（既存キャッシュは force で再生成）。バカンス個体の日記の認知負荷分離（現地=ありのまま・機械=昇華）が背景
