...
- アルバム機能（v3.52）— 画像記憶システム新規実装。MCPツール4本（`album_save`/`album_read`/`album_list`/`album_share`）。URL直リンクまたはNASローカルから画像取得→長辺1024pxリサイズ→`/data/album/`に保存。MCP image content type 対応。REST 7本（一覧・画像返却・アップロード・メタデータ更新・削除・共有URL生成・共有画像返却）。admin.html Album タブ（サムネイルグリッド・アップロード・編集・削除・共有）
