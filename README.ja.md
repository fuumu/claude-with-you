...
- アルバム機能（v3.51）— 画像記憶システム新規実装。MCPツール4本（`album_save`/`album_read`/`album_list`/`album_share`）。URL直リンクまたはNASローカルから画像取得→長辺1024pxリサイズ→`/data/album/`に保存。MCP image content type 対応。REST `GET /api/album/`・`/api/album/{id}`・共有URL
