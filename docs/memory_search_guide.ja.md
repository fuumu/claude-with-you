...
> **v3.81〜：** `search_layer` パラメータで検索対象レイヤーを明示的に限定可能。`index`（1層のみ）/`summary`（2層のみ）/`full`（全文のみ）。省略時は従来通り階層フォールスルー。`author` パラメータで著者フィルタも可。

> **v3.41（M2）〜：** 3層 symbolic（象徴的記述）は index.json に収載され、**1次の高速検索（keywords と同じ層）でヒット対象**になりました。symbolic のみで当たった場合 `match_layer="symbolic"` が付きます（keywords 一致は `"keyword"`）。つまり「具体語（keywords）」でも「意味の本質（symbolic）」でも、本文を読まずに引けます。
...
| 問いの種類 | 強い層 | `search_layer` 指定 | 理由 |
|---|---|---|---|
| 技術用語・バージョン・ツール名 | keywords | `index` | 固有名詞が直接入る |
| 「〜について話した」 | keywords / summary | 省略（階層探索） | トピックが入る |
| 「あの議論の詳細は？」 | full body | `full` | 細部が必要 |
| 「意味のある瞬間を探す」 | タイトル/タグ（手書きメモ） | `index` | キュレーション済み |
...
> **複合キーワードはAND検索（v3.48）**：クエリをスペース（半角・全角）で区切ると、各語が全層で個別にAND判定される。例：`友達システム テスト` は両方を含むエントリだけにヒット。語を足すほど絞り込める。単語1つなら従来の部分一致と同じ。
