...
```json
{
  ...
  "serverInfo": { "name": "mio-memory", "version": "3.17.0" },
...
返却は body の代わりに `summary`（2層要約）＋ `match_layer`（keyword/summary/full）。
...
※ 友達セッションは `/mcp?token=<friend_token>` でアクセスした場合のみ有効。通常の17ツールは使用不可。
