...
- 並び順・最小メッセージ数・レーティング（v3.70）・ソース（チャット / Claude Code / OpenWebUI、`source` フィールド判定、v3.72）: クライアント側で適用
...
#### admin.html バックアップUI（v3.64・B1-UI）

Import タブ末尾の「バックアップ（CoreMem＋ExtMemory）」セクション。API は v3.63 のまま変更なし。

- **取得側**: 「📦 バックアップZIPをダウンロード」ボタン。`GET /api/export?token=<token>` の
  クエリトークン認証（Uploads タブの認証付きダウンロードと同方式）でブラウザ直ダウンロード
- **復元側（2段階フロー必須）**: ZIP ドラッグ&ドロップ／ファイル選択 → mode 選択
  （skip=既存を守る（デフォルト）／overwrite=上書き・各説明文付き）→
  「1. プレビュー（dry run）」で `dry_run=true` を実行し件数テーブル（ExtMemory／CoreMem ×
  復元/スキップ/上書き）と衝突一覧（折りたたみ）を表示 → ユーザーが確認して
  「2. 復元を実行」で本実行。**プレビューを通さず本実行はできない**
  （実行ボタンはプレビュー成功後のみ表示、mode を変更すると再度プレビューが必要）。
  overwrite の本実行には追加で confirm ダイアログを挟む
- i18n（日英・`backup.*` キー）・モバイルレスポンシブ（結果テーブルは横スクロール）対応
- 狙い: curl 不要でバックアップ運用が完結すること。特に災害復旧時（新環境の直後）は
  コマンドラインが使えない状況がありうるため、ブラウザだけで戻せる導線を確保する
...
| POST | `/api/import/openwebui` | admin | OpenWebUI チャットエクスポート `.json` 取り込み（`overwrite=true` で再処理）（v3.66） |

- Claude Code: `.zip` の場合は再帰的に `.jsonl` を収集（`subagents/` ディレクトリ配下は除外）
- セッションID（ファイル名 / chat ID）をキーに `imported_uuids.json` で重複チェック（ZIP インポートと共通）
- 会話ごとに ExtMemory エントリを作成：
  - Claude Code: タイトル `[会話/Code] {title}`、タグ `["会話ログ", "claude-code", "raw"]`、`author: "claude-code"`
  - OpenWebUI: タイトル `[会話/OpenWebUI] {title}`、タグ `["会話ログ", "openwebui", "raw"]`、`author: "openwebui"`
- 取り込み成功後は要約バッチを自動起動（ZIP インポートと同じ挙動）

### OpenWebUI インポートの変換仕様（v3.66）

- OpenWebUI エクスポート JSON は配列形式（各要素が `{id, title, chat: {messages, history, models, ...}}` の構造）
- `chat.messages` 配列がある場合はそれを優先。なければ `chat.history.messages` からタイムスタンプ順でメッセージを再構成
- UNIX タイムスタンプ（秒 / ミリ秒）を JST の ISO 形式に変換
- トップレベルに `source: "openwebui"` を付与
- admin.html Import タブにドロップゾーン追加（ブラウザからインポート可能）
...
- ~~夜間バッチでのローカル LLM による自動分類~~ → v3.68 セーフチェックバッチで実装済み
- ~~admin.html Logs タブのレーティング表示・設定 UI~~ → v3.70 logs.html で実装済み（バッジ・フィルタ・手動上書き）
- inbox メッセージへの適用
...
| `file_read` | メタデータ返却。テキスト系はcontentフィールドに本文含む（50K文字で打ち切り）。判定は mimetype（text/*・application/json 等）＋拡張子フォールバック（json/jsonl/yaml/py 等17種、v3.66） |
...
## 20.5. TS-0: API契約ドキュメント化＋特性テストスイート（2026-07-13）

TS-1（TypeScript移行・ストラングラー方式）の前提となる「現行挙動の固定化」。単独でも回帰テストとして価値がある。

### 設計方針

- **HTTP越しブラックボックス** — テストは main.py の内部を import しない。REST と MCP JSON-RPC
  を実際に叩いて応答形状を assert する。サーバー実装を差し替えても（TS版でも）
  conftest の起動コマンドを変えるだけでスイート全体がそのまま「同一サーバー」判定に使える
- **既存環境に触れない** — conftest がテストごとに一時データディレクトリ＋空きポートで
  サーバーをサブプロセス起動する。本番NASに対して実行するものではない
- 要約バッチが実LMStudioへ接続しないよう、テスト時は到達不能な宛先を環境変数で注入する

### テスト用フック（本体変更・後方互換）

| 変更 | 内容 |
|---|---|
| `MIO_DATA_ROOT` | `/data` 直書きだった全パス定数を1変数から導出（デフォルト `/data`・運用不変） |
| `MIO_PORT` | リッスンポート（デフォルト `5002`・運用不変） |
| symlink コピーフォールバック | CoreMem の最新版リンク（`_link_or_copy_latest`）が symlink 非対応環境（特権なしWindows）でファイルコピーに退避。`_artifacts_list` は非リンク実体ファイルも versions/ から版数を導出して一覧に含める。Linux 運用は従来どおり symlink |

### カバレッジ（53テスト）

認証（Bearer/クエリ/401）・OAuthディスカバリ・MCPトランスポート（initialize/tools一覧31本/
...
source_thread自動紐づけ・上書き禁止）・会話ログ（search/index/read スライス/注記/レーティングゲート）・
Uploads/Album REST・バッチstatus・export/reindex。
未カバー領域は docs/api-contract.ja.md §8 に明記。

## 21. 統合検索（v3.61）

### 設計方針

「認証まわりはどう決めたっけ？」のような探索で、記憶（ExtMemory）と会話ログ（LogStore）を
別々のツールで二度引く手間をなくす（淳さん提案・2026-06-20）。
`memory_search` / `GET /api/memory/hsearch` に `include_conversations`（デフォルト false・
後方互換）を追加し、true のとき会話ログのタイトル検索結果を併せて返す。

### 挙動

- 記憶の階層検索（既存）はそのまま。追加で `_load_conv_index()` の各会話タイトルに
  同じAND判定（`_query_terms` / `_all_terms_in`）を適用
- ヒットした会話は `updated_at` 降順にソートし、`limit` 件まで `conversations[]`
  （uuid・title・created_at・updated_at・message_count）として返す。総数は `conversations_total`
- `rating=adult` の会話は `include_adult=true` のときのみ含める（v3.56 レーティング保護と整合）
- 検索対象はタイトルのみ（本文検索は全会話ファイルの読み込みが必要で重いため対象外。
  本文まで探す場合は従来どおり `conversation_search` → `conversation_read` を使う）

## 22. admin.html Import タブ：Claude Code ログ取り込みUI

v3.54 の `POST /api/import/claude-code` はREST直叩きのみだったため、admin.html の
Import タブに専用ドロップゾーンを追加。

- 既存の claude.ai エクスポートZIP用ドロップゾーンの下に、Code ログ用の
  第2ドロップゾーン（🛠）を配置
- `.jsonl` の複数選択（1ファイルずつ順次POST）と `.zip` の一括投入に対応
- 「上書きモード」チェックボックスは両ドロップゾーン共通
- 結果表示は imported / skipped / errors / 紐づけ（source_threads_linked）の合算
- 取り込み後は既存の要約バッチ進捗パネルをポーリング（自動起動されたバッチが見える）

### admin.html Memoryタブ：生ログへのリンク

記憶エントリの詳細モーダルに、`source_thread` が設定されていれば「📖 生ログを開く」
リンクを表示する（Filesタブの「生まれた会話を開く」と同じ `openConvInLogs` パターン）。
`logs.html?conv=<uuid>` ディープリンクで Logs タブの該当会話へジャンプする。
...
v3.73 でルールに禁止表現リスト（性器名称・行為進行・体液・体位の具体記述）を追加し、
代替表現の例（波・潮・溶ける・重なる等）を明示。昇華プロンプトに「最重要原則: 必ず
mature 以下」「出力前自己検証」を追加。

### セルフチェックループ（`_sublimate_chunk`）

昇華出力をレーティング判定に通す。v3.73 で判定を **strict モード** に分離:

- `_judge_rating_single(strict=True)`: temperature=0 で決定的に動作
- 昇華出力専用の厳格プレフィクス（「このテキストは昇華済み。残存描写がないか厳格に…」）
- 生成と判定が同一モデルでも「引きずられ」を低減する設計

フロー:
1. 昇華 → strict 判定。mature 以下なら完了
2. adult なら具体的な指摘をフィードバックして再昇華（上限2回）
...
クライアント切断後もLLMジョブが走り続ける問題への対策:

- **入力サイズ上限**: 60,000文字（`_SUBLIMATE_MAX_INPUT_CHARS`）。超過時は即エラーで
  `msg_from`/`msg_to` 範囲の縮小を推奨
- **処理時間上限**: 180秒（`_SUBLIMATE_TIMEOUT_SEC`）。超過時は処理済みチャンクの
  partial 結果を返却（`partial=true` / `chunks_completed` / `timeout_message`）
- 各チャンクの LLM タイムアウトは `_lm_client()` の 300s が個別の安全弁
...
【生】タイトルの inbox_post（to=chat）着弾時に、サーバー内完結で自動退避＋昇華を実行する
パイプライン。友達トークン経由のローカル委託方式（F4 未完了で採用見送り）の代替。

**トリガー**: `_inbox_needs_sublimate(to, title)` — `to == "chat"` かつ
タイトルに `【生】` を含む場合。`_post_inbox_message` 内で検知。

**処理フロー**（`_post_inbox_message` 内の同期＋非同期の2段階）:

1. **同期（即時・生テキスト滞留ゼロ）**:
   - 原文を ExtMemory に `rating=adult`・`local_only=True`・タグ `["バカンス日記", "自動退避"]`
     で保存（`_create_raw_backup_entry`）。本文先頭に来歴コメント（inbox_id・投函日時）
   - inbox メッセージの本文をプレースホルダに差替え（`【処理中】昇華処理を実行中です。
     原文は ExtMemory {memory_id} に退避済みです。`）
   - タイトルの `【生】` を `【処理中】` に置換
   - **要件4**: この時点で生テキストは inbox から完全に消えている

2. **非同期（daemon スレッド）**（`_run_inbox_sublimate`）:
   - `_sublimate_text(original_body)` を実行（既存のチャンク分割・セルフチェックループ）
   - 成功（mature 以下）: 本文を昇華版に差替え、タイトルを `【未承認】` に変更、
     末尾に原文 memory_id・レーティング・チャンク数を記載
   - 失敗（`needs_human` / adult 残留）: 本文を通知のみに差替え（memory_id 参照）、
     タイトルを `【要人手】` に変更。**生テキストを inbox に残さない**
   - 例外: 同上（`【要人手】`＋例外メッセージ）

**承認フロー**: 自動化は「下書きまで」。【未承認】→【承認済み】への昇格は個体本人が
