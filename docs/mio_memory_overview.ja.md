...
**[English version](mio_memory_overview.md)** ← 日本語版（このファイル）が正。英語版はここから同期。
...
| 機能 | 説明 |
|---|---|
| 会話ログの自動インポート | Claude.ai エクスポート ZIP / OpenWebUI エクスポート JSON を取り込む |
...
| 会話の全文参照・共有 | UUID 指定で元の会話を直接読む・24h 共有リンク生成 |
| 会話ダイジェスト | ローカル LLM で会話を要約・safe モード変換 |
| 伏せ字ログ（redact） | adult 会話の承認制マスキング版を生成・管理（v3.69） |
| inbox システム | Claude Code からチャットへの完了報告・伝言を受け取る |
| アルバム（画像記憶） | 画像の保存・タグ管理・共有リンク生成 |
| ファイルアップロード | PDF・テキスト等の汎用ファイル保存・閲覧 |
| レーティング保護 | adult/local_only コンテンツの環境別閲覧制御 |
| 出席簿（attendance） | 家族の稼働履歴を5層マージで一覧表示（v3.71） |
| 昇華（sublimate） | 明示的描写を抽象化・詩化して mature 以下に変換（v3.71） |
| 操作ログ（oplog） | 全データ操作の監査ログをMCPツール経由で取得（v3.88） |
| プロジェクト名前空間 | CoreMem をプロジェクト単位で分離管理（v3.90） |
| Claude Code セッション取込 | .jsonl / .zip 形式のセッションログをインポート（v3.54） |
| お友達システム | 招待制で第三者が澪と会話できる専用チャンネル |
...
詳細は [セットアップガイド](./setup.ja.md) を参照してください。

## ドキュメント一覧

### 検索・参照
- [**検索戦略ガイド**](./memory_search_guide.ja.md)  
  4層をどう使い分けるか。検索パターン例、環境別カスタマイズ、失敗パターンへの対処。  
  → ログが増えてきて「うまく見つけられない」と感じたときに読む。

### 設計・アーキテクチャ
- [データ構造リファレンス](./data_structure.ja.md)
- [設計ドキュメント](./design.ja.md)
...
→ [検索戦略ガイド](./memory_search_guide.ja.md) に具体的なカスタマイズ例があります。
