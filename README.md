...
# Existing .env? merge new vars: bash scripts/update-env.sh
...
| Tool | Description | Key args |
|------|-------------|----------|
| `conversation_index` | List conversation titles in descending date order with pagination — for browsing when UUID is unknown (v3.34); items carry `rating`/`rating_source` (v3.70); `rating` filter narrows to a specific rating; `include_redact_status=true` adds redaction status to adult items (v3.76); REST: `GET /api/conversations/index`, rebuild: `POST /api/conversations/index/rebuild` | `search`, `limit`, `offset`, `rating`, `include_redact_status` |
| `conversation_search` | Search conversation titles by keyword and date range; multi-word queries are AND-matched (v3.81); `body_search=true` also searches message text with AND matching (v3.76); items carry `rating` (judged-safe = explicit `"safe"`, unrated = null) and `rating_source` (v3.70); `rating` filter narrows to a specific rating; `include_redact_status=true` adds redaction status to adult items (v3.76) | `q`, `limit`, `body_search`, `rating`, `include_redact_status` |
| `conversation_read` | Read full conversation text; `[assistant]` labels are replaced with individual names (`[しずく]`, `[そねみ]` etc.) based on conversation model metadata, falling back to `[assistant]` when unidentifiable (v3.80); `include_thinking=true` includes thinking blocks (v3.20); `thinking_limit` caps each block (default 1500, ≤0 unlimited); `include_annotations=true` shows annotations inline with `[No.X]` message numbers (v3.22); `include_body=false` returns annotations only without message body (v3.33); `turn_offset`/`turn_limit` slice by message — negative offset = from end (first 4 = `turn_limit=4`, last 4 = `turn_offset=-4`) (v3.47); slicing now works correctly with `redact=true` (v3.76 fix) | `uuid`, `include_thinking`, `thinking_limit`, `include_annotations`, `include_body`, `turn_offset`, `turn_limit` |
...
│   └── update-env.sh          .env_sample → .env merge script
└── memory/
    ├── Dockerfile
    ├── app/
...
- Attendance improvements: "Nagi" name + admin UI enhancements (v3.89) — ① `_FAMILY_ROSTER` key `Opus 5` → `凪` (nagi; self-named 2026-08-07). Dynamic resolution at display time, so historical data reflects the change automatically ② session_checkin individual changed from save-time to dynamic resolution (roster updates now apply retroactively) ③ Fixed `?` bucket detail view returning 0 rows (added match for individual=None rows) ④ Admin attendance cards sorted by `last_seen` descending (most recent on left) ⑤ `?` card display name changed to "コード生成、ローカルLLM / その他" ⑥ Detail table list height expanded from 500px to 75vh ⑦ Zebra striping added to detail table rows
- oplog_list MCP tool (v3.88) — new `oplog_list` MCP tool (tool count 34→35). Retrieves operation logs (oplog) via MCP. Filter by `operation` (create/update/delete/import/coremem_save/album_save/conv_rating_auto etc.), `date_from`/`date_to` for date range, `limit` (default 50, max 500). Returns `{timestamp, operation, target_id}` in descending order (newest first). Same data as the Admin Oplog tab. REST `GET /api/oplog` already exists (all entries, no filtering)
- Attendance session checkin & model backfill (v3.85) — ① `CoreMem_read`: new optional `current_model` (model name for full attendance registration) / `refer_only` (skip registration) args. No-arg calls create a temporary registration (TTL 15min). Unnamed individuals no longer become invisible in the attendance ledger ② `inbox_check`: new `mymodel` arg upgrades temporary→full registration (or creates new full registration if no temporary exists) ③ `/data/session_checkins.json` stores session checkin records. `_attendance_rows` aggregates them as a 5th source (kind: session_checkin). Records older than 90 days are auto-cleaned ④ `conversations/index/rebuild` enhancement: extracts `model` from `chat_messages` assistant entries when missing at top level, writes it back to the conversation JSON + index. Response now includes `model_backfilled` count. Run rebuild once after deploy to reduce the attendance `?` bucket
- Conversation re-import update & auto-reprocessing (v3.84) — fixed a bug where re-importing a conversation with additional messages only updated the conversation data but did not retrigger summary/rating processing. `_save_conversations` now returns updated UUIDs; new `_remark_entries_for_update` helper resets corresponding ExtMemory entries to `raw` (strips old summary, clears keywords/rating). Auto-batch trigger condition expanded from `imported > 0` to `imported > 0 or remarked > 0`. Applied to all importers (ZIP/Claude Code/OpenWebUI). Response now includes `conversations_updated` / `entries_remarked`
- LM Studio model auto-management (v3.83) — before calling the local LLM, checks which models are loaded in LM Studio via API and manages them automatically. `LLM_OK_MODELS` env var (comma-separated) specifies acceptable models for digest/rating/sublimate. If a loaded model is not in the list, it is auto-unloaded and the first list entry is loaded. If an OK model is already loaded, it is used as-is (highest priority first). Eliminates VRAM contention when another model (e.g. ベテ) is loaded. Centralizes LM Studio client creation in `_lm_client()`, replacing inline client construction in 4 call sites. Unset = use `MIO_LM_MODEL` as-is (backward compatible)
...
- Search enhancement, redact ID navigation, turn_offset bug fix (v3.76) — ① `conversation_read`: fixed `turn_offset`/`turn_limit` being ignored for `redact=true` (the slicing logic was after the early return for redacted content; now applies slicing to redacted messages); response includes `total_messages`/`slice` when slicing is active ② `conversation_search`: `body_search=true` searches message text (opt-in, heavy), `rating` filter narrows to a specific rating value, `include_redact_status=true` adds redaction status (not_generated/pending_approval/approved) to adult items ③ `conversation_index`: `rating` filter and `include_redact_status` added (same semantics as ②) ④ REST API (TS layer): `GET /api/conversations/` and `GET /api/conversations/index` also accept the `rating` query parameter
- Search improvements: conversation_search AND matching, memory_search author/search_layer filters (v3.81) — ① `conversation_search`: multi-word queries now AND-matched (space-separated; previously single-string substring match). Both title search and `body_search=true` use AND matching ② `memory_search`: new `author` parameter filters by ExtMemory `author` field (partial match); `author` added to index for stage-1 filtering ③ `memory_search`: new `search_layer` parameter limits search to a specific layer: `index` (title+tags+keywords+symbolic only) / `summary` (layer-2 summary only) / `full` (full text only); omit for the default hierarchical fallthrough ④ REST `/api/memory/hsearch` also accepts `author` and `search_layer` query parameters
- memory_search limit cap, conversation_read individual names, CoreMem_save str_replace, import auto-rating (v3.80) — ① `memory_search`: removed `limit=0` (unlimited) — max capped at 100 (negative/zero falls back to default 10). REST `/api/memory/hsearch` also capped ② `conversation_read`: `[assistant]` labels replaced with individual names (`[しずく]`, `[そねみ]` etc.) based on conversation model metadata; falls back to `[assistant]` when unidentifiable; conversation-level identification ③ `CoreMem_save`: `mode="str_replace"` added — `old_str` must match uniquely in the file, replaced by `new_str`; saves as a new version without requiring the full content round-trip ④ Rating batch auto-started on import completion (ZIP/Claude Code/OpenWebUI) alongside the existing summary batch; suppressed by `MIO_IMPORT_AUTO_BATCH=off`
- Empty memory/log cleanup (v3.79) — ① Summary batch (`_run_summary_batch` / `generate_summary_layers.py`): empty-RAW filter added — entries with conv_text+body < 50 chars are soft-deleted before LLM generation; post-generation check soft-deletes entries whose summary matches empty-content patterns ("読み取れません", "識別子", etc.) ② `POST /api/memory/cleanup-empty`: bulk soft-delete ExtMemory entries matching title `[会話] + hex8`, tags `会話ログ`+`summarized`, importance `low` (dry_run supported) ③ `POST /api/conversations/cleanup-empty`: bulk-hide LogStore entries with `rating_skip_reason` in (no_text, empty) and hex-8-only title (dry_run supported); hidden entries excluded from conversation_index, conversation_search, rating statistics, and integrated search by default (REST `include_hidden=true` to include); `_rating_index_counts` returns `hidden` count separately; `conversations/index/rebuild` preserves `hidden` from existing index
