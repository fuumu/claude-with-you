...
**MCP tools exposed (v3.68 — 32 regular-session tools):**
...
- `batch_run_rating` — start the rating batch to auto-assign `rating` (safe/mature/adult) + `rating_reason` to unrated conversation logs using local LLM; `status_only=true` returns progress + `pending` count; `force=true` re-judges `auto` entries (never touches `manual`); chunks long conversations and takes the highest rating; adds `rating_source`/`rating_judged_at`/`rating_model` metadata; REST `GET /api/rating-batch/status`, `POST /api/rating-batch/start` (v3.68)
