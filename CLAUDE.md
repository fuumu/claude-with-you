...
- `inbox_check` / `inbox_read` / `inbox_post` / `inbox_update` / `inbox_delete` — lightweight inter-session messaging (`/data/inbox/`); `inbox_post(persistent=true)` creates standing messages that never get marked as read; `inbox_post` accepts `from_model`/`to_model` as string or array (v3.57, array enables multi-name tagging e.g. `["claude-opus-4-6", "しずく"]`); `inbox_check` returns `persistent[]{id, title, body, created_at, from_model, to_model}` with full bodies (v3.20, no `inbox_read` calls needed for standing messages) plus `non_persistent_unread_count`/`non_persistent_unread_ids`; `inbox_check(include_read=true)` additionally returns all messages with `messages[]{id, read, persistent, title, from, to, from_model, to_model}` and `unread_count`; `inbox_check` supports `limit`/`days`/`from_model`/`to_model` filters (v3.57); `inbox_update(id, persistent?, title?, body?)` partial-updates a message; `inbox_delete(id)` physically deletes a message (irreversible)
