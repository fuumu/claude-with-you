...
**MCP tools exposed (v3.72 — 34 regular-session tools):**
...
- `sublimate` — sublimation transform (v3.71): rewrites text (or a conversation excerpt via `uuid` + `msg_from`/`msg_to`) preserving emotional temperature/meaning while abstracting explicit act descriptions, targeting mature-or-below; style rules unified in `_SUBLIMATION_STYLE_RULES` (single source, also drives digest safe_mode); output is self-checked through the rating judge — adult results are re-sublimated (max 2 retries) then returned with `needs_human=true`; long input chunks at paragraph boundaries (~6000 chars); LLM: `MIO_LM_MODEL` via LMStudio
