# protocol_guide.md — MCPツール運用ガイド（mio-memory v3.56 / 全25本）
...
| 21 | `album_save` | Album | 画像を保存（URL/NASパス、自動リサイズ） | 中 |
| 22 | `album_read` | Album | 画像を取得（base64＋メタデータ） | 中（画像分） |
| 23 | `album_list` | Album | 画像メタデータ一覧（本体なし） | 軽 |
| 24 | `album_share` | Album | 画像の24h共有URL | 軽 |
| 25 | `album_delete` | Album | 画像とメタデータを完全削除 | 軽 |

※ 友達セッション（`/mcp?token=<friend_token>`）では別途の限定ツールが出る。本ガイドは通常セッション25本が対象。
