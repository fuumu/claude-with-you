# protocol_guide.md — MCPツール運用ガイド（mio-memory v3.61 / 全31本）
...
**`memory_search`** — `q`(必須)・`limit`(既定10,0=無制限)・`offset`・`full_body`・`include_local`・`include_adult`・`include_conversations`。**階層検索**：1次=index（title+tags+keywords+3層symbolic）→ 2次=要約 → 3次=全文。返り値は `summary`＋`symbolic`、各ヒットに `match_layer`（keyword/symbolic/summary/full）。全文は `full_body=true` か `memory_read`。`local_only`／`adult` はデフォルト除外（v3.56）。`include_conversations=true` で会話タイトルも検索し `conversations[]` を併せて返す（統合検索・v3.61）。**軽〜中**。
