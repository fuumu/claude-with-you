...
変���履歴:
  ...
  v3.73 (2026-07-19) - memory_upsert rating温存 + sublimate昇華強化・孤児ジョブ対策
    ...
    - sublimate: 入力サイズ上限(60000文字)＋処理時間上限(180秒)→partial結果返却で孤児ジョブ防止
...
def _judge_rating_single(client, model, conv_text, strict=False):
    """単一チャンクのレーティング判定。(rating, reason) を返す。
    strict=True: 昇華出力の検証用。より厳格に判定し temperature=0 で決定的に動作する"""
    strict_prefix = ('注意: このテキストは昇華（抽象化）済みの出力です。'
                     '元が adult だったため、残存する具体的描写がないか厳格に判定してください。'
                     '行為の進行が追える描写、性器の示唆、体液の記述が一つでもあれば adult です。\n\n') if strict else ''
    prompt = strict_prefix + _RATING_PROMPT_TEMPLATE + conv_text
...
- 以下は明確に禁止（出力に一つでも含まれれば adult 判定になる）:
  - 性器・性感帯の直接的な名称や婉曲でも特定できる指示語
  - 挿入・愛撫・オーガズム等の行為進行を具体的に追える描写
  - 体液・身体反応（濡れる、硬くなる等）の直接的記述
  - 行為中の体位・動作の具体的な記述
- 代わりに使える表現: 波・潮・溶ける・重なる・満ちる・深く沈む、など行為を特定できない抽象語"""

_SUBLIMATION_PROMPT_TEMPLATE = """あなたはテキストの「昇華」変換を行います。昇華とは、体験の温度・感情・意味を保ったまま、性的・身体的な行為の明示的描写を抽象化・詩化することです。

## 最重要原則
出力は必ず mature 以下（性行為の明示的描写ゼロ）でなければならない。一文でも具体的な行為描写が残っていれば失敗。迷ったら抽象化しすぎる側に倒す。
...
- 出力前に自己検証: 「性行為を具体的に追える描写が一文でも残っていないか？」を確認すること
{feedback}
以下のテキストを昇華してください:
...
def _sublimate_chunk(client, model, text):
    ...
    for attempt in range(1, 4):  # 初回 + 再昇華2回
        ...
        feedback = (f'\n※前回の昇華結果はまだ adult 判定でした（理由: {reason}）。'
                    '具体的な行為描写・身体部位の記述を完全に除去し、抽象的な表現に置き換えてください。\n')
    return out, rating, reason, 3, True


_SUBLIMATE_MAX_INPUT_CHARS = 60000  # 同期処理の入力上限（約20ターン相当）
_SUBLIMATE_TIMEOUT_SEC = 180  # 全体の処理時間上限


def _sublimate_text(text):
    """テキストを昇華する。長文は段落境界でチャンク分割して個別に昇華→結合。
    全体レーティングは各チャンクの最高値を採用する。
    処理時間上限を超えた場合は処理済みチャンクのpartial結果を返す"""
    import time as _time
    if len(text) > _SUBLIMATE_MAX_INPUT_CHARS:
        return {'error': (f'入力が大きすぎます（{len(text)}文字、上限{_SUBLIMATE_MAX_INPUT_CHARS}文字）。'
                          'msg_from/msg_to で範囲を狭めてください'),
                'input_chars': len(text), 'max_chars': _SUBLIMATE_MAX_INPUT_CHARS}
    ...
    timed_out = False
    for c in chunks:
        if _time.time() - t_start > _SUBLIMATE_TIMEOUT_SEC:
            timed_out = True
            break
    ...
    if timed_out:
        ...
        result['timeout_message'] = (
            f'処理時間上限（{_SUBLIMATE_TIMEOUT_SEC}秒）超過。'
...
_MCP_TOOLS = [
    ...
    {
        "name": "memory_upsert",
        "description": "固定IDで記憶エントリを上書きする（存在しない場合は新規作成）。core.mdなど固定キーの更新に使う。rating/local_only未指定時は既存値を温存する",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "tags":       {"type": "array", "items": {"type": "string"}},
                "importance": {"type": "string", "enum": ["high", "normal", "low"]},
                "rating":     {"type": "string", "enum": ["safe", "mature", "adult"], "description": "レーティング（省略時は既存値を温存）"},
                "local_only": {"type": "boolean", "description": "ローカル専用フラグ（省略時は既存値を温存）"}
            },
...
def _handle_tool_call_raw(name, arguments):
    if name == "memory_read_index":
        ...
        # rating / local_only: 引数指定時はその値、未指定時は既存値を温存（v3.73）
        rating_arg = arguments.get("rating")
        if rating_arg in ("safe", "mature", "adult"):
            entry["rating"] = rating_arg
        elif before and before.get("rating"):
            entry["rating"] = before["rating"]
        local_only_arg = arguments.get("local_only")
        if local_only_arg is not None:
            if local_only_arg:
                entry["local_only"] = True
        elif before and before.get("local_only"):
            entry["local_only"] = True
        with open(path, "w") as f:
            json.dump(entry, f, ensure_ascii=False, indent=2)
